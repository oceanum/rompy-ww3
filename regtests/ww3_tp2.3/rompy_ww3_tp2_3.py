#!/usr/bin/env python3
"""
WW3 tp2.3 regression test configuration using rompy-ww3.

Test: 2-D propagation on Cartesian grid - "Garden Sprinkler Effect".
...
def create_ww3_tp2_3_components():
    """Create rompy-ww3 components for tp2.3 Garden Sprinkler Effect test."""
    ...
    grid_component = Grid(
        ...
        rect=Rect(
            nx=48,
            ny=38,
            sx=100000.0,  # 100 km spacing
            sy=100000.0,
            x0=-600000.0,  # -600 km origin
            y0=-600000.0,
        ),
        depth=Depth(
...
def main():
    """Generate WW3 configuration for tp2.3 regression test."""
    print("Creating WW3 configuration for ww3_tp2.3 regression test...")
    print("Test: 2-D propagation on Cartesian grid - Garden Sprinkler Effect")

    components = create_ww3_tp2_3_components()
    config = NMLConfig(**components)
    period = TimeRange(start="1968-06-06T00:00:00", duration="5D", interval="1H")

    model_run = ModelRun(
        run_id="ww3_tp2_3_regression",
        config=config,
        period=period,
    ...
    print("=" * 80)
    print("\nGenerated namelists for tp2.3 test:")
    print("  - ww3_shel.nml: Main shell configuration")
    print("  - ww3_grid.nml: Grid preprocessing (48x38 CART)")
    print("  - namelists.nml: Physics parameters")
    print("  - ww3_ounf.nml: Field output")
    print("\nThis test validates 2-D wave propagation - Garden Sprinkler Effect.")
