...
from .namelists import Domain, Input, OutputType, OutputDate, HomogCount, HomogInput
...
class NamelistComposition(BaseModel):
    ...
    domain: Optional[Domain] = Field(
        default=None, description="DOMAIN_NML configuration"
    )
    input_nml: Optional[Input] = Field(
        default=None, description="INPUT_NML configuration"
    )
    output_type: Optional[OutputType] = Field(
        default=None, description="OUTPUT_TYPE_NML configuration"
    )
    output_date: Optional[OutputDate] = Field(
        default=None, description="OUTPUT_DATE_NML configuration"
    )
    homog_count: Optional[HomogCount] = Field(
        default=None, description="HOMOG_COUNT_NML configuration"
    )
    homog_input: Optional[List[HomogInput]] = Field(
        default=None, description="HOMOG_INPUT_NML configurations"
    )
    ...
    def validate_homog_input(cls, v, values):
        ...
        if v is not None and "homog_count" in values:
            ...
            if homog_count is not None:
                ...
                if (
                    homog_count.n_wnd is not None
                    and input_counts.get("WND", 0) != homog_count.n_wnd
                ):
                    logger.warning(
                        f"Mismatch between HOMOG_COUNT%N_WND ({homog_count.n_wnd}) and actual WND inputs ({input_counts.get('WND', 0)})"
                    )
