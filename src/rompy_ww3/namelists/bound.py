...
class Bound(NamelistBaseModel):
    ...
    mode: Optional[str] = Field(
        default=None,
        description="Mode: 'WRITE' or 'READ'"
    )
    interp: Optional[int] = Field(
        default=None,
        description="Interpolation method: 1 (nearest), 2 (linear)"
    )
    verbose: Optional[int] = Field(
        default=None,
        description="Verbosity level: 0, 1, or 2"
    )
    file: Optional[str] = Field(
        default=None,
        description="Input spec.nc listing file"
    )