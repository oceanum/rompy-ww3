...
class Point(NamelistBaseModel):
    ...
    list: Optional[str] = Field(
        default=None, description="List of points index ['all'|'1 2 3']"
    )
    samefile: Optional[bool] = Field(
        default=None, description="All the points in the same file"
    )
    ...
    dimorder: Optional[bool] = Field(
        default=None, description="[time,station=T | station,time=F]"
    )
