...
class PointOutput(NamelistBaseModel):
    ...
    field_list: Optional[str] = Field(
        default=None, description="List of fields to output at points (space-separated)"
    )

    # File configuration
    single_file: Optional[bool] = Field(
        default=None, description="Flag to output all points to single file (T/F)"
    )
    buffer_size: Optional[int] = Field(
        default=None, description="Number of points to process per pass"
    )

    # Interpolation
    interp_method: Optional[str] = Field(
        default=None, description="Interpolation method ('nearest', 'linear')"
    )
