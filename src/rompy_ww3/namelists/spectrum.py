...
class Spectrum(NamelistBaseModel):
    ...
    def validate_xfr(cls, v):
        ...
        if v is not None:
            if v <= 1.0:
                raise ValueError(
                    f"Frequency increment factor (xfr) must be > 1.0, got {v}"
                )
            if v > 2.0:  # Reasonable upper limit
                raise ValueError(
                    f"Frequency increment factor (xfr) seems too high: {v}"
                )
        return v
    ...
    def validate_nk(cls, v):
        ...
        if v is not None:
            if v < 2:
                raise ValueError(
                    f"Number of frequency bins (nk) must be at least 2, got {v}"
                )
        return v
    ...
    def validate_nth(cls, v):
        ...
        if v is not None:
            if v < 2:
                raise ValueError(
                    f"Number of direction bins (nth) must be at least 2, got {v}"
                )
        return v
    ...
    def validate_thoff(cls, v):
        ...
        if v is not None:
            if v < -0.5 or v > 0.5:
                raise ValueError(
                    f"Direction offset (thoff) must be between -0.5 and 0.5, got {v}"
                )
        return v
