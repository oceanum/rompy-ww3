...
def validate_ww3_boolean(value: str) -> str:
    ...
    if upper_value not in WW3_BOOLEAN_VALUES:
        raise ValueError(f"Invalid WW3 boolean value: '{value}'. Must be one of {WW3_BOOLEAN_VALUES}")
...
def validate_io_type(value: int) -> int:
    ...
    if value not in IOSTYP_VALUES:
        raise ValueError(f"IOSTYP value {value} is invalid. Must be one of {sorted(list(IOSTYP_VALUES))}")
    return value
...
def validate_grid_type(value: str) -> str:
    ...
    if upper_value not in GRID_TYPE_VALUES:
        raise ValueError(f"Invalid grid type: '{value}'. Must be one of {sorted(list(GRID_TYPE_VALUES))}")
...
def validate_coord_type(value: str) -> str:
    ...
    if upper_value not in COORD_TYPE_VALUES:
        raise ValueError(f"Invalid coordinate type: '{value}'. Must be one of {sorted(list(COORD_TYPE_VALUES))}")
...
def validate_clos_type(value: str) -> str:
    ...
    if upper_value not in CLOS_TYPE_VALUES:
        raise ValueError(f"Invalid grid closure type: '{value}'. Must be one of {sorted(list(CLOS_TYPE_VALUES))}")
...
def validate_forcing_type(value: str) -> str:
    ...
    if upper_value not in FORCING_VALUES:
        raise ValueError(f"Invalid forcing type: '{value}'. Must be one of {sorted(list(FORCING_VALUES))}")
    
    return upper_value


def validate_positive_value(value: Union[int, float], field_name: str) -> Union[int, float]:
...
def validate_non_negative_value(value: Union[int, float], field_name: str) -> Union[int, float]:
...
def validate_direction_range(value: Union[int, float], field_name: str) -> Union[int, float]:
...
def validate_frequency_range(value: Union[int, float], field_name: str) -> Union[int, float]:
    ...
    if value <= 0 or value > 1.0:  # WW3 typically uses frequencies < 1 Hz
        raise ValueError(f"{field_name} value {value} is out of reasonable range (0, 1.0]")
    return value
...
def validate_direction_bins(value: int, field_name: str) -> int:
    ...
    if value <= 0 or value > 720:  # Maximum practical direction bins
        raise ValueError(f"{field_name} value {value} is out of reasonable range (0, 720]")
    return value
...
def validate_frequency_bins(value: int, field_name: str) -> int:
    ...
    if value <= 0 or value > 100:  # Maximum reasonable frequency bins
        raise ValueError(f"{field_name} value {value} is out of reasonable range (0, 100]")
    return value